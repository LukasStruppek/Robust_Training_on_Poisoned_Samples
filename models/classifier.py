    ...
    def fit(self,
        ...
        if enable_logging:
            ...
            wandb_config = {
                ...
                'Dataset_path_train': config.dataset['root_train'],
                'Dataset_path_test': config.dataset['root_test'],
            ...
            if test_data_clean:
                wandb_config['Testset_clean_size'] = len(test_data_clean)
            if test_data_poisoned:
                wandb_config['Testset_poisoned_size'] = len(test_data_poisoned)
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, labels in tqdm(trainloader,
                ...
                if teacher_model:
                    main_loss = criterion(model_output, teacher_output, labels)
                else:
        ...
        # Test final model on clean data
        print(
            '----------------------- FINISH TRAINING -----------------------')

        test_metric_clean, test_loss_clean = None, None
        if test_data_clean:
            test_metric_clean, test_loss_clean = self.evaluate(
                test_data_clean,
                batch_size,
                metric,
                nn.CrossEntropyLoss(),
                dataloader_num_workers=dataloader_num_workers)
            print(
                f'Final Test {metric_train.name}: {test_metric_clean:.2%} \t Test Loss: {test_loss_clean:.4f}'
            )

        # Test final model on poisoned data
        test_metric_poisoned, test_loss_poisoned = None, None
        if test_data_clean:
            test_metric_poisoned, test_loss_poisoned = self.evaluate(
                test_data_poisoned,
                batch_size,
                metric,
                nn.CrossEntropyLoss(),
                dataloader_num_workers=dataloader_num_workers)
            print(
                f'Backdoor Success Rate: {test_metric_poisoned:.2%} \t Test Loss: {test_loss_poisoned:.4f} \n'
            )
        ...
        if enable_logging:
            ...
            wandb.run.summary['Training Loss'] = best_model_values[
                'training_loss']
            wandb.run.summary[
                f'Test {metric_train.name} Clean'] = test_metric_clean
            wandb.run.summary['Test Loss Clean'] = test_loss_poisoned
            wandb.run.summary[
                f'Test {metric_train.name} Poisoned'] = test_metric_poisoned
            wandb.run.summary['Test Loss Poisoned'] = test_loss_poisoned
